...
- **Ограничения API**  
  - Операции закрытия сессии выделены в отдельные группы лимитов, и их значения увеличены до уровня в два раза выше, чем лимиты для операций открытия сессии:
    - интерактивная сессия: `20/60/240`,
    - пакетная сессия: `20/40/120`.
  - Ответ GET `/rate-limits` дополнен группой `anonymous`, содержащей лимиты для анонимных операций API. Изменение имеет организационный характер — лимиты уже действовали ранее, но не возвращались этим endpoint.
  - Ответ GET `/rate-limits` расширен группой `global`, предназначенной для **будущих глобальных лимитов API**, начисляемых на IP-адрес. Механизм в настоящее время отключен и не влияет на интеграции. Он будет применяться в основном к поставщикам приложений, обслуживающим большое количество субъектов, и направлен на повышение безопасности их интеграций. Подробности работы и график включения будут представлены в отдельном сообщении.

- **Отправка счетов в интерактивной сессии (POST `/sessions/online/{referenceNumber}/invoices`)**  
  Добавлен код ошибки `21184` ("Сессия временно недоступна"), возвращаемый с HTTP 400 в случае временного приостановки возможности отправки счетов в рамках существующей сессии. Рекомендуется открыть новую сессию и продолжить отправку.

- **Отправка счетов**  
  На тестовых средах добавлена возможность включения валидации номеров NIP и внутренних идентификаторов субъектов с помощью заголовка `X-KSeF-Feature`: `subject-identifier-validation`.

- **OpenAPI**  
  - Расширен словарь кодов стран значениями: `CNH`, `VED`, `XTS`, `ZWG`, `SLE`; изменение касается областей "Получение счетов" и "Коллективные идентификаторы".
  - Дополнены ограничения длины `PublicKeyCertificate.publicKeyId` - добавлены `minLength/maxLength: 44`.
  - Обновлена определение `ForbiddenProblemDetails` для ответа 403 - удален timestamp из списка полей `required`. Изменение адаптирует спецификацию к реальному поведению API, в том числе для `security-service-blocked`.
  - Небольшие обновления описаний и примеров.

- **Тестовые данные**  
  Дополнена документация ответа `400 Bad Request` для POST и DELETE `/testdata/limits/context/session` стандартным кодом ошибки `21405` ("Ошибка валидации входных данных").
...
- **Сводные идентификаторы**  
  - Изменен endpoint GET `/collective-identifiers/{collectiveIdentifierNumber}/invoices` на POST `/collective-identifiers/invoices`. Метод принимает список до 10 сводных идентификаторов вместо одного номера. Ответ дополнен `collectiveIdentifierNumber`, увеличен максимальный `pageSize` с 200 до 500, endpoint включен в API limits и добавлен пример ответа.
  - Увеличены API limits с `10/60/120` до `20/120/240` для всех endpoint'ов группы `collectiveIdentifier`.
  - Добавлены примеры ответов.

- **Лимиты сводных идентификаторов**  
  Расширены лимиты текущего контекста на `collectiveIdentifier.maxInvoices` - максимальное количество счетов в одном ИЗ. Лимит возвращается GET `/limits/context`, а на среде TEST может быть изменен через POST `/testdata/limits/context/session`.

- **Экспорт пакета счетов (POST `/invoices/exports`). Получение списка метаданных счетов (POST `/invoices/query/metadata`)**    
  Увеличен максимальный диапазон dateRange с 3 месяцев до 100 дней в формате UTC. Изменение обратно совместимо.

- **Получение статуса экспорта пакета счетов (GET `/invoices/exports/{referenceNumber}`)**  
  Ответ дополнен свойством `compressionType`, указывающим тип сжатия пакета.  

- **OpenAPI**  
  Дополнено описание `X-System-Warning` о способе тестирования предупреждений с использованием заголовка `X-Test-System-Warning`.
...
- **Получение статуса аутентификации (GET `/auth/{referenceNumber})`**  
  Унифицированы сообщения в поле `details` для статуса `450` («Аутентификация завершена неудачей из‑за неверного токена») — содержимое теперь возвращается на польском языке, в соответствии с документацией. Кроме того, дополнена документация несколькими вариантами `details`, которые встречались в ответах API, но ранее не были описаны.

- **Экспорт пакета счетов (POST `/invoices/exports`)**  
  Исправлено разделение пакетов в режиме `onlyMetadata=true` для экспортов, включающих более 10 000 счетов — ранее файл `_metadata.json` мог содержать 10 001 запись в одном пакете, а `isTruncated` оставался `false`. Пакеты теперь разделяются корректно, а значение `isTruncated` отражает реальное состояние результата.
...
| Среда | Дата развертывания |
| ---------- | -------------: |
| **TEST**   |     19.05.2026 |
| **DEMO**   |     21.05.2026 |
| **PRD**    |     26.05.2026 |

- **Пакетная отправка (POST `/sessions/batch`) и экспорт пакета накладных (POST `/invoices/exports`)**  
  Добавлен тип сжатия `TarGz` как альтернатива `Zip`. Формат `TarGz` рекомендуется из-за возможности достижения лучшего коэффициента сжатия для пакетов, содержащих много похожих XML-документов. Типом сжатия по умолчанию остается `Zip` для сохранения совместимости.

- **Технические предупреждения (`X-System-Warning`)**  
  Добавлен опциональный заголовок ответа `X-System-Warning`, позволяющий передавать технические предупреждения без влияния на результат операции (например, когда обнаружено поведение, которое в будущем может привести к отклонению запроса). В среде TEST предупреждение можно принудительно вызвать заголовком `X-Test-System-Warning`.
...
- **Публичные ключи KSeF (ротация и выбор ключей)**  
  Введена передача селектора `publicKeyId` в запросах к операциям, в которых клиент шифрует данные публичным ключом KSeF - это позволяет однозначно идентифицировать используемый ключ и обеспечить корректную и автоматическую обработку ре-сертификации/ротации. Расширение касается endpoints: 
    - POST `/auth/ksef-token`, 
    - POST `/sessions/online`, 
    - POST `/sessions/batch`, 
    - POST `/invoices/exports`.  
  
  Дополнительно дополнена [документация](bezpieczenstwo/klucze-publiczne-do-szyfrowania.md) описанием публикации и ротации публичных ключей.

- **Аутентификация с использованием подписи XAdES (POST `/auth/xades-signature`)**  
  - Расширена поддержка документа `AuthTokenRequest` версией схемы 2.1 (при сохранении совместимости со схемой 2.0).
  - В схеме 2.1 обновлены регулярные выражения для IP-адресов (`Ip4Address`, `Ip4Range`, `Ip4Mask`), чтобы они соответствовали типичным парсерам.

- **Лимиты API**  
  Выравнены лимиты API по умолчанию в среде TEST со значениями, действующими в PRD. В среде TEST по-прежнему доступны endpoints из группы `/testdata/rate-limits`, позволяющие тестировать нестандартные профили лимитов.
...
| Среда | Дата развертывания | Примечания                                                            |
| ---------- | -------------: | ---------------------------------------------------------------- |
| **TEST**   |     10.04.2026 | —                                                                |
| **DEMO**   |     13.04.2026 | —                                                                |
| **PRD**    |     16.04.2026 | Ужесточение валидации XML начнет действовать с **16.07.2026**. |


- **Токены KSeF**
  - Разрешено выполнение операций с токеном, используемым для аутентификации, без дополнительных разрешений:
    - DELETE `/tokens/{referenceNumber}` позволяет аннулировать собственный токен (тот, которым произошла аутентификация) без `CredentialsManage`,
    - GET `/tokens` и GET `/tokens/{referenceNumber}` возвращают информацию о токене, используемом для аутентификации, также когда он не обладает разрешениями `CredentialsManage` / `CredentialsRead`.
  - Обновление описаний:
    - DELETE `/tokens/{referenceNumber}`,
    - GET `/tokens/`

- **Отправка накладных**  
  - Ужесточена проверка содержимого XML: 
    - документ не может содержать нерекомендуемые символы Unicode, указанные в спецификации XML W3C,
    - документ не может содержать инструкции обработки XML (processing instructions).  

    Подробности: [Проверка накладной](/faktury/weryfikacja-faktury.md).

- **Хранение статусов асинхронных операций**  
  Введено хранение для технических ответов и статусов асинхронных операций (служащих для краткосрочного опроса прогресса и получения результата). Хранение не касается бизнес-данных, являющихся результатом выполнения операций. После истечения периода хранения опрос статуса возвращает `410 Gone`.
  Действующие периоды хранения:
    - Аутентификация - GET `/auth/{referenceNumber}`: 7 дней,
    - Экспорт накладных - GET `/invoices/exports/{referenceNumber}`: 7 дней,
    - Выдача сертификата - GET `/certificates/enrollments/{referenceNumber}`: 30 дней,
    - Предоставление/получение разрешений - GET `/permissions/operations/{referenceNumber}`: 30 дней.  
    
  Для статусов сессий и отправки накладных хранение в настоящее время не планируется.

- **OpenAPI**  
  - Предоставлен опциональный формат ошибок Problem Details (`application/problem+json`) для ответов `400 Bad Request` и `429 Too Many Requests` (согласованный с форматом, уже используемым для `401` и `403`). Формат можно включить заголовком `X-Error-Format: problem-details`; существующие ответы `application/json` остаются поддерживаемыми. 
  - Добавлена поддержка ответов `410 Gone` в формате Problem Details (`application/problem+json`).
  - В ответах `400` в формате Problem Details возвращается список `errors` (без агрегирования нескольких ошибок в одну позицию).
  - Расширены `ForbiddenProblemDetails` и `UnauthorizedProblemDetails` свойством `timestamp` ("Дата и время возникновения ошибки в UTC.").
  - Уточнены ограничения для свойства `AllowedIps` в модели запроса POST `/auth/ksef-token` - добавлены атрибуты `minimum` и `maximum` (лимит элементов списков для `ip4Addresses`, `ip4Ranges`, `ip4Masks`). Изменение носит документационный характер: лимиты применялись и ранее, а их превышение приводило к ошибке валидации.
  - Мелкие обновления описаний и примеров.

- **Лимиты API**  
  Увеличены лимиты для POST `/invoices/exports`, выравнивая их до значений, действующих для POST `/invoices/query/metadata`:
    - req/s: с 4 до 8,
    - req/min: с 8 до 16.
