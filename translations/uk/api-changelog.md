...
- **API ліміти**  
  - Відокремлено операції закриття сесій у окремі групи лімітів і збільшено їх значення до рівня вдвічі вищого, ніж ліміти для операцій відкриття сесій:
    - інтерактивна сесія: `20/60/240`,
    - пакетна сесія: `20/40/120`.
  - Додано до відповіді GET `/rate-limits` групу `anonymous`, що містить ліміти для анонімних операцій API. Зміна має характер упорядковуючий — ліміти вже діяли раніше, але не поверталися цим endpoint.
  - Розширено відповідь GET `/rate-limits` групою `global`, призначеною для **майбутніх глобальних лімітів API**, що нараховуються за IP-адресою. Механізм наразі вимкнений і не впливає на інтеграції. Він стосуватиметься переважно постачальників додатків, що обслуговують велику кількість суб'єктів, і має на меті підвищення безпеки їхніх інтеграцій. Деталі роботи та графік увімкнення будуть представлені в окремому повідомленні.

- **Відправка рахунків-фактур у інтерактивній сесії (POST `/sessions/online/{referenceNumber}/invoices`)**  
  Додано код помилки `21184` ("Сесія тимчасово недоступна"), що повертається з HTTP 400 у разі тимчасового призупинення можливості відправки рахунків-фактур в рамках існуючої сесії. Рекомендується відкрити нову сесію та продовжити відправку.

- **Відправка рахунків-фактур**  
  На тестових середовищах додано можливість увімкнення валідації номерів NIP та ідентифікаторів внутрішніх суб'єктів за допомогою заголовка `X-KSeF-Feature`: `subject-identifier-validation`.

- **OpenAPI**  
  - Розширено словник кодів країн значеннями: `CNH`, `VED`, `XTS`, `ZWG`, `SLE`; зміна стосується областей "Отримання рахунків-фактур" та "Колективні ідентифікатори".
  - Додано обмеження довжини `PublicKeyCertificate.publicKeyId` - додано `minLength/maxLength: 44`.
  - Оновлено визначення `ForbiddenProblemDetails` для відповіді 403 - видалено timestamp зі списку полів `required`. Зміна адаптує специфікацію до реальної поведінки API, зокрема для `security-service-blocked`.
  - Незначні оновлення описів та прикладів.

- **Тестові дані**  
  Додано документацію відповіді `400 Bad Request` для POST та DELETE `/testdata/limits/context/session` зі стандартним кодом помилки `21405` ("Помилка валідації вхідних даних").
...
- **Збірні ідентифікатори**  
  - Змінено endpoint GET `/collective-identifiers/{collectiveIdentifierNumber}/invoices` на POST `/collective-identifiers/invoices`. Метод приймає список до 10 збірних ідентифікаторів замість одного номера. Відповідь розширено на `collectiveIdentifierNumber`, збільшено максимальний `pageSize` з 200 до 500, endpoint охоплено обмеженнями API та додано приклад відповіді.
  - Збільшено обмеження API з `10/60/120` до `20/120/240` для всіх endpointів з групи `collectiveIdentifier`.
  - Додано приклади відповідей.

- **Обмеження збірних ідентифікаторів**  
  Розширено обмеження поточного контексту на `collectiveIdentifier.maxInvoices` - максимальну кількість рахунків-фактур в одному ЗІ. Обмеження повертається через GET `/limits/context`, а на середовищі TEST може бути змінено через POST `/testdata/limits/context/session`.

- **Експорт пакета рахунків-фактур (POST `/invoices/exports`). Отримання списку метаданих рахунків-фактур (POST `/invoices/query/metadata`)**    
  Збільшено максимальний діапазон dateRange з 3 місяців до 100 днів, що відраховуються в UTC. Зміна є зворотно сумісною.

- **Отримання статусу експорту пакета рахунків-фактур (GET `/invoices/exports/{referenceNumber}`)**  
  Розширено відповідь властивістю `compressionType`, що вказує тип стиснення пакета.  

- **OpenAPI**  
  Доповнено опис `X-System-Warning` про спосіб тестування попереджень з використанням заголовка `X-Test-System-Warning`.
...
- **Колективні ідентифікатори**  
  Додано підтримку колективних ідентифікаторів та новий дозвіл `CollectiveIdentifierManage`, необхідний для виконання операцій над ІЗ.

- **Тестові дані**  
  Додано endpoint PUT `/testdata/certificates/{serialNumber}`, який дозволяє скоротити термін дії сертифіката KSeF шляхом зміни `validTo` для симуляції сценаріїв закінчення терміну дії сертифіката на тестових середовищах.

- **OpenAPI**  
  Уніфіковано описи необхідних дозволів: коли достатньо мати один із кількох дозволів, використовується формулювання "Потрібен один із дозволів", а в разі одного дозволу — "Потрібен дозвіл". Зміна має характер документаційний; поведінка API залишається без змін.
...
- **Пакетне надсилання (POST `/sessions/batch`) та експорт пакету рахунків-фактур (POST `/invoices/exports`)**  
  Додано тип стиснення `TarGz` як альтернативу до `Zip`. Формат `TarGz` рекомендується завдяки можливості досягнення кращого коефіцієнта стиснення для пакетів, що містять багато схожих XML-документів. Типом стиснення за замовчуванням залишається `Zip` для збереження сумісності.

- **Технічні попередження (`X-System-Warning`)**  
  Додано необов'язковий заголовок відповіді `X-System-Warning`, що дозволяє передавати технічні попередження без впливу на результат операції (наприклад, коли виявлено поведінку, яка в майбутньому може призвести до відхилення запиту). У середовищі TEST попередження можна примусово викликати заголовком `X-Test-System-Warning`.
...
- **Публічні ключі KSeF (ротація та вибір ключів)**  
  Запроваджено передачу селектора `publicKeyId` у запитах до операцій, у яких клієнт шифрує дані публічним ключем KSeF — це дозволяє однозначно ідентифікувати використаний ключ та забезпечити коректну й автоматичну обробку ре-сертифікації/ротації. Розширення стосується таких endpoint-ів:
    - POST `/auth/ksef-token`,
    - POST `/sessions/online`,
    - POST `/sessions/batch`,
    - POST `/invoices/exports`.  
  
  Додатково в [документацію](bezpieczenstwo/klucze-publiczne-do-szyfrowania.md) додано опис публікації та ротації публічних ключів.

- **Автентифікація з використанням підпису XAdES (POST `/auth/xades-signature`)**  
  - Розширено підтримку документа `AuthTokenRequest` до версії схеми 2.1 (зі збереженням сумісності зі схемою 2.0).
  - У схемі 2.1 оновлено регулярні вирази для IP-адрес (`Ip4Address`, `Ip4Range`, `Ip4Mask`), щоб вони відповідали поширеним парсерам.

- **Ліміти API**  
  Ліміти API за замовчуванням у середовищі TEST зрівняно зі значеннями, що діють у PRD. У середовищі TEST як і раніше доступні endpoint-и групи `/testdata/rate-limits`, що дозволяють тестувати нестандартні профілі лімітів.
...
| Середовище | Дата впровадження | Примітки                                                                                   |
| ---------- | ----------------: | ------------------------------------------------------------------------------------------ |
| **TEST**   |        10.04.2026 | —                                                                                          |
| **DEMO**   |        13.04.2026 | —                                                                                          |
| **PRD**    |        16.04.2026 | Посилення валідації XML набере чинності з **16.07.2026**. |


- **Токени KSeF**
  - Надано можливість виконувати операції з токеном, використаним для автентифікації, без додаткових прав доступу:
    - DELETE `/tokens/{referenceNumber}` дозволяє анулювати власний токен (той, за допомогою якого відбулася автентифікація) без права `CredentialsManage`,
    - GET `/tokens` та GET `/tokens/{referenceNumber}` повертають інформацію про токен, використаний для автентифікації, навіть якщо він не має прав `CredentialsManage` / `CredentialsRead`.
  - Оновлення описів:
    - DELETE `/tokens/{referenceNumber}`,
    - GET `/tokens/`

- **Надсилання рахунків-фактур**  
  - Посилено перевірку вмісту XML:
    - документ не може містити небажані символи Unicode, зазначені в специфікації XML W3C,
    - документ не може містити інструкції обробки XML (processing instructions).  

    Деталі: [Верифікація рахунку-фактури](/faktury/weryfikacja-faktury.md).

- **Утримання статусів асинхронних операцій**  
  Запроваджено утримання технічних відповідей і статусів асинхронних операцій (призначених для короткострокового опитування прогресу та отримання результату). Утримання не стосується бізнес-даних, що є результатом виконання операції. Після закінчення терміну утримання опитування статусу повертає `410 Gone`.  
  Чинні терміни утримання:
    - Автентифікація — GET `/auth/{referenceNumber}`: 7 днів,
    - Експорт рахунків-фактур — GET `/invoices/exports/{referenceNumber}`: 7 днів,
    - Видача сертифіката — GET `/certificates/enrollments/{referenceNumber}`: 30 днів,
    - Надання/відкликання прав — GET `/permissions/operations/{referenceNumber}`: 30 днів.  
    
  Для статусів сесій і надсилання рахунків-фактур утримання наразі не планується.

- **OpenAPI**  
  - Надано необов'язковий формат помилок Problem Details (`application/problem+json`) для відповідей `400 Bad Request` і `429 Too Many Requests` (узгоджений з форматом, що вже використовується для `401` і `403`). Формат можна увімкнути за допомогою заголовка `X-Error-Format: problem-details`; попередні відповіді `application/json` залишаються підтримуваними.
  - Додано підтримку відповіді `410 Gone` у форматі Problem Details (`application/problem+json`).
  - У відповідях `400` у форматі Problem Details повертається список `errors` (без агрегування кількох помилок в один запис).
  - Розширено `ForbiddenProblemDetails` і `UnauthorizedProblemDetails` властивістю `timestamp` ("Дата та час виникнення помилки в UTC.").
  - Уточнено обмеження для властивості `AllowedIps` у моделі запиту POST `/auth/ksef-token` — додано атрибути `minimum` і `maximum` (ліміт елементів списків для `ip4Addresses`, `ip4Ranges`, `ip4Masks`). Зміна має документаційний характер: ліміти застосовувалися й раніше, а їх перевищення призводило до помилки валідації.
  - Незначні оновлення описів і прикладів.

- **Ліміти API**  
  Збільшено ліміти для POST `/invoices/exports`, вирівнявши їх до значень, що діють для POST `/invoices/query/metadata`:
    - req/s: з 4 до 8,
    - req/min: з 8 до 16.
